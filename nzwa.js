...
async function starts() {
	...
	nzwa.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⏳ Espera ai tome um cafe☕  com Vrau⏳',
				success: '✔️ Funciona âœ”ï¸',
                                levelon: '✔️ *habilitar nivelamento*',
				leveloff: ' ✔️  *desabilitar nivelamento*',
				levelnoton: '❌­ *a nota de nivelamento está ativa*',
				...
				only: {
					...
					Badmin: '[❕] Este comando só pode ser usado quando o bot se torna administrador!',
                                        daftarB: `──「 AINDA NAO REGISTRADO 」──\nOla irmao !\nVoce ainda nao Registre-se aqui, register vamos primeiro... \n\nComando : ${prefix}register nome|era\nExemplo : ${prefix}register Vrau|16`,
				}
			}
               ...
               break
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Rebaixado com sucesso @${mentioned[0].split('@')[0]} Menjadi Member Group!`)
						nzwa.groupDemoteAdmin(from, mentioned)
					}
					break
                                case 'randomhentai':
                  ...
                  case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Pedido recebido, adicionar posicao como administrador : @${mentioned[0].split('@')[0]}`, mentioned, true)
						nzwa.groupMakeAdmin(from, mentioned)
					}
					break
				  case 'wa.me':
    ...
    nzwa.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
               ...
               nzwa.sendMessage(from, 'Aqui está o número do meu dono, mano, salve mano, salve depois ',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                nzwa.updatePresence(from, Presence.composing) 
                if (!isRegister) return reply(mess.only.daftarB)
              ...
              await nzwa.sendMessage(from, options, text)
               break
				...
				case 'marvellogo':
					...
					if (args.length < 1) return reply(`Kirim perintah ${prefix}marvellogo teks, contoh ${prefix}marvellogo Lorde Canss`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(mess.wait)
		...
		case 'infonomor':
               nzwa.updatePresence(from, Presence.composing) 
                 if (!isRegister) return reply(mess.only.daftarB)
                 if (isLimit(sender)) return reply(ind.limitend(pusname))
                 if (args.length < 1) return reply(`Insira numeros\nExemplo : ${prefix}infonomor 0888345678`)
                data = await fetchJson(`https://docs-jojo.herokuapp.com/api/infonomor?no=${body.slice(11)}`)
                if (data.error) return reply(data.error)
                if (data.result) return reply(data.result)
                hasil = `â• âž¥ internacional : ${data.international}\nâ• âž¥ numero : ${data.nomor}\nâ• âž¥ operator : ${data.op}`
                reply(hasil)
                await limitAdd(sender)
                break
                   case 'map':
				...
				case 'ytmp3':
					...
					teks = `â *Titulo* : ${anu.title}\nâ *Tamanho Arquivo* : ${anu.filesize}\n\nEspere Um Minuto Ok, Audio Enviado Novamente...`
	...
	case 'kbbi':
	      ...
	      tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'adminlist':
					nzwa.updatePresence(from, Presence.composing) 
                                        if (!isRegister) return reply(mess.only.daftarB)
					if (!isGroup) return reply(mess.only.group)
					teks = `Lista admin deste grupo *${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
				...
				case 'block':
					...
					nzwa.sendMessage(from, `Pedidos aceitos, memblokir ${body.slice(8)}@c.us`, text)
					break
				case 'hilih':
					nzwa.updatePresence(from, Presence.composing) 
				...
				nzwa.updatePresence(from, Presence.composing) 
					...
					nzwa.sendMessage(from, 'â•”â•â•âœªã€˜ Mencione tudo ã€™âœªâ•â•\nâ• âž¥'+teks+'â•šâ•ã€˜ - - - - ã€™', text, {quoted: mek})
					break
                case 'tagall3':
				nzwa.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				case 'alay':
					...
					nzwa.groupLeave (from) 
                     ...
                     setTimeout( () => {
					nzwa.updatePresence(from, Presence.composing) 
					nzwa.sendMessage(from, 'AdeusðŸ‘‹', text) // ur cods
					}, 0)
                     break

				case 'chord':
					if (args.length < 1) return reply('onde está o título da música mana')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					tels = body.slice(7)					
			...
			case 'igstalk':
               ...
               nzwa.updatePresence(from, Presence.composing) 
              options = {
          text: `Este Dono desta Merda : @${from.split("-")[0]}`,
          contextInfo: { mentionedJid: [from] }
           ...
           break
				...
				case 'kick':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Pedidos recebidos, emitidos removi esta pessoa porque nao transa  :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Pedido recebido, removi esta pessoa porque nao transa : @${mentioned[0].split('@')[0]}`, mentioned, true)
						nzwa.groupRemove(from, mentioned)
					nzwa.sendMessage(mentioned, 'Adeus pessoa feiaðŸ˜‚', text)
					}
					break
				case 'exe':
	              nzwa.updatePresence(from, Presence.composing) 
	              if (!isOwner) return reply(mess.only.ownerB)
	               ...
	               exec(cmd, (err, stdout) => {
		           if(err) return nzwa.sendMessage(from, "Comando Incorreto", text, { quoted: mek })
		           if (stdout) {
				...
				case 'ocr':
                    ...
                    reply('O problema foi relatado ao proprietário do BOT, relatório falso/main2 tidak akan ditanggapi.')
                    break
               case 'apakah':
               nzwa.updatePresence(from, Presence.composing) 
	    ...
	    case 'kapankah':
			...
			case 'closegc':
                ...
                case 'bukagc':
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					open = {
					text: `Grupo aberto pelo administrador @${sender.split("@")[0]}\nagora *podem falar membros comum* ghosts e manifestem`,
					contextInfo: { mentionedJid: [sender] }
					}
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('âŒ Falha ao converter o adesivo em imagemr âŒ')
                	...
                	nzwa.updatePresence(from, Presence.composing) 
                        if (!isRegister) return reply(mess.only.daftarB)
					if (!isQuotedVideo) return reply('âŒ responder vídeo hum âŒ')
					reply(mess.wait)
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await nzwa.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp4')
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('âŒ Falha ao converter vídeo para mp3 âŒ')
						buffer = fs.readFileSync(ran)
	          ...
	          if (!isRegister) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'logowolf':
					...
					if (args.length < 1) return reply(`teksnya mana? contoh ${prefix}logowolf Vrau|Lords`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(mess.wait)
					...
					if (Number(args[0]) === 1) {
						...
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/json/nsfw.json', JSON.stringify(nsfw))
						reply('â¬ SUCESSO â­ desative o recurso nsfw neste grupo')
					} else {
						reply('digite 1 para ativar, 0 para desativar o recurso')
					}
					break	
				...
				case 'logowolf2':
					...
					var teks1 = gh.split("|")[0];
					var teks2 = gh.split("|")[1];
					if (args.length < 1) return reply(`teksnya mana? contoh ${prefix}logowolf Cafea|Canss`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(mess.wait)
					...
					var gbl1 = gh.split("|")[0];
					var gbl2 = gh.split("|")[1];
					if (args.length < 1) return reply('Cadê o texto, hum\nExemplo: ${prefix}phlogo |Vrau|Canss')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(mess.wait)
					...
					break
                case 'truth':
					const trut =['Sempre como qualquer um? Quao mais?','Kalau boleh atau kalau mau, di gc/luar gc siapa yang akan kamu jadikan sahabat?(boleh beda/sma jenis)','apa ketakutan terbesar kamu?','pernah suka sama orang dan merasa orang itu suka sama kamu juga?','Siapa nama mantan pacar teman mu yang pernah kamu sukai diam diam?','pernah gak nyuri uang nyokap atau bokap? Alesanya?','hal yang bikin seneng pas lu lagi sedih apa','pernah cinta bertepuk sebelah tangan? kalo pernah sama siapa? rasanya gimana brou?','pernah jadi selingkuhan orang?','hal yang paling ditakutin','siapa orang yang paling berpengaruh kepada kehidupanmu','hal membanggakan apa yang kamu dapatkan di tahun ini','siapa orang yang bisa membuatmu sange','siapa orang yang pernah buatmu sange','(bgi yg muslim) pernah ga solat seharian?','Siapa yang paling mendekati tipe pasangan idealmu di sini','suka mabar(main bareng)sama siapa?','pernah nolak orang? alasannya kenapa?','Sebutkan kejadian yang bikin kamu sakit hati yang masih di inget','pencapaian yang udah didapet apa aja ditahun ini?','kebiasaan terburuk lo pas di sekolah apa?']
            ...
            break
				        ...
				        uptime = process.uptime()
                                ...
                                case 'neonlogo':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('onde está o texto hum\nExemplo: ${prefix}neonlogo Vrauuu')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'neonlogo2':
                                        ...
                                        teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('onde está o texto hum\nExemplo: ${prefix}neonlogo2 Lordecafeanss')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'lionlogo':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply('onde está o texto hum\nExemplo: ${prefix}lionlogo Lordrcoffe|Canss')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'jsholat':
                                        loc = body.slice(8)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (args.length < 1) return reply('Onde fica a área mana?')
                                ...
                                case 'jokerlogo':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('onde está o texto hum\nExemplo: ${prefix}jokerlogo Tiocafeanss')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
			       ...
			       reply(anu.result.jadwalTV)
					break*/
                                ...
                                case 'woodblock':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('onde está o texto hum\nExemplo: ${prefix}woodblock TioCafss')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'qowheart':
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        nzwa.sendMessage(from, buffer, image, {quoted: mek, caption: 'Tai a foto..'})
                                        await limitAdd(sender)
                                        break
                                case 'mutgrass':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('teksnya mana um\nContoh: ${prefix}mutgrass Tiocafenss')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        nzwa.sendMessage(from, buffer, image, {quoted: mek, caption: 'ok envie o comando correto...'})
                                        await limitAdd(sender)
                                        break
                                case 'undergocean':
                                        var gh = body.slice(12)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('teksnya mana um\nContoh: ${prefix}undergocean Vrau')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                        anu = await fetchJson(`https://tobz-api.herokuapp.com/api/photooxy?theme=underwater_ocean&text=${teks1}&apikey=BotWeA`, {method: 'get'})
                                        buffer = await getBuffer(anu.result)
                                        nzwa.sendMessage(from, buffer, image, {quoted: mek, caption: 'ok envie correto...'})
                                        await limitAdd(sender)
                                        break
                                case 'woodenboards':
                                        var gh = body.slice(13)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('teksnya mana um\nContoh: ${prefix}woodenboards Vrau')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'metalictglow':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('onde está o texto hum\nExemplo: ${prefix}metalictglow Vrau')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
				...
				case 'simi':
					...
					//if (anu.error) return reply('Simi Não sei mana')
					reply(anu)
					break
				case 'simih':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('tipo 1 para ativar o recurso')
					if (Number(args[0]) === 1) {
						if (isSimi) return reply('O modo Simi está ativo')
						samih.push(from)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
						reply('Ativar com sucesso o modo simi neste grupo âœ”ï¸')
					} else if (Number(args[0]) === 0) {
						samih.splice(from, 1)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
				...
				case 'clone':
					...
					try {
						...
						mentions(`Foto de Perfil atualizada com sucesso usando a foto de perfil @${id.split('@')[0]}`, [jid], true)
					} catch (e) {
						reply('Om falha')
					}
					break
                                case 'pubglogo':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply('teksnya mana um\nContoh: ${prefix}pubglogo Vrau|Canss')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        nzwa.sendMessage(from, buffer, image, {quoted: mek, caption: 'Aqui está o logotipo, mana...'})
                                        await limitAdd(sender)
                                        break
                                case 'herrypotter':
                                case 'harrypotter':
                                        var gh = body.slice(12)
                                        if (args.length < 1) return reply('onde está o texto hum\nContoh: ${prefix}harrypotter Vrau|ss')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        anu = await fetchJson(`https://tobz-api.herokuapp.com/api/photooxy?theme=harry_potter&text=${gh}&apikey=BotWeA`, {method: 'get'})
                                        buffer = await getBuffer(anu.result)
                                        nzwa.sendMessage(from, buffer, image, {quoted: mek, caption: 'Aqui esta a foto...'})
                                        await limitAdd(sender)
                                        break
                                case 'katabijak':
                                        if (!isRegister) return reply(mess.only.daftarB)
                                ...
                                case 'bal':
                                        ...
                                        break
                                case 'buylimit':
                                        if (args.length < 1) return reply('Qual o limite que voce quer comprar mana? Certifique-se de ter dinheiro suficiente, mana! \n\nCara cek uang: ${prefix}bal')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                ...
                                case 'event':
                                        ...
                                        if (Number(args[0]) === 1) {
                                                ...
                                                reply('*â¬ SUCESSO â­ Ative EVENT neste grupo*')
                                        } else if (Number(args[0]) === 0) {
                                                event.splice(from, 1)
                                                fs.writeFileSync('./database/json/event.json', JSON.stringify(event))
                                                reply('*â¬ SUCESSO â­ Desative EVENT neste grupo*')
                                        } else {
                                                reply(ind.satukos())
                                        }
                                        break
                                case 'register':
                                        ...
                                        if (isGroup) {
                                                ...
                                                console.log(color('[REGISTRO]'), color(time, 'yellow'), 'Nome:', color(namaUser, 'cyan'), 'Era:', color(umurUser, 'cyan'), 'Serial:', color(serialUser, 'cyan'), 'in', color(sender || groupName))
                                        } else {
                                                addRegisteredUser(sender, namaUser, umurUser, time, serialUser)
                                                await reply(ind.registered(namaUser, umurUser, serialUser, time, sender))
                                                addATM(sender)
					...
					break*/
                                case 'translate':
                                        ...
                                        arteks = `â—ª *TRADUZIR* \n  â”‚\n  â”œâ”€ â Texto : ${teksnya} \n  â”œâ”€ â Traduzir : ${anu.text} \n  â””â”€ â *Pronunciation* : ${anu.pronunciation}`
					...
					if (Number(args[0]) === 1) {
						...
						nzwa.sendMessage(from,`Atencao a todos os membros ativos deste grupo anti-link. se você enviar um link de grupo, voce sera expulso daqui do grupo`, text)
					} else if (Number(args[0]) === 0) {
				...
				default:
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[ADVERTIR]','red'), 'Comando não registrado de', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
